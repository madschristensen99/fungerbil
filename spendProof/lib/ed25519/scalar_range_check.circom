// scalar_range_check.circom - Verify scalar is in valid Ed25519 range
// Ensures scalar < L where L is the Ed25519 curve order
...
// Verify that a 255-bit scalar is less than L (Ed25519 curve order)
// This prevents invalid scalar attacks
template ScalarInRange() {
    signal input s[255];  // Input scalar as 255 bits (LSB first)
    
    // L = 2^252 + 27742317777372353535851937790883648493
    // L in hex = 0x1000000000000000000000000000000014def9dea2f79cd65812631a5cf5d3ed
    // L in bits (little-endian, 255 bits - top bit is always 0 for valid scalars)
    var L_bits[255];
    ...
    // Byte 31 (bits 248-254): 0x10 = 00010000 (only 7 bits, bit 255 not used)
    L_bits[248] = 0; L_bits[249] = 0; L_bits[250] = 0; L_bits[251] = 0;
    L_bits[252] = 1; L_bits[253] = 0; L_bits[254] = 0;
    
    // Compare s < L using bit-by-bit comparison (lexicographic order)
    // We need to check if s < L by comparing from most significant bit down
    
    // Convert to numbers for comparison
    component s_num = Bits2Num(255);
    component L_num = Bits2Num(255);
    ...
    // Use LessThan to verify s < L
    component lt = LessThan(252);  // L is 252 bits + small constant
    lt.in[0] <== s_num.out;
    lt.in[1] <== L_num.out;
    
    // Constrain that s must be less than L
    lt.out === 1;
}
